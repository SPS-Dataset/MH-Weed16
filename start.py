...
#parser.add_argument('-o', '--INPUT_DIR', default='output', type=str, help='Path to output directory')
parser.add_argument('-t', '--thickness', default='2', type=int, help='Bounding box and cross line thickness')
parser.add_argument('-s', '--steps', default='1', type=int, help='Bounding box and cross line thickness')
...
parser.add_argument('-c','--cfg', default= NULL , type=str, help='Path of cfg file  ')
parser.add_argument('-w','--weights', default=NULL , type=str, help='Path of weights file')
...
#INPUT_DIR  = "./input/"+args.input_dir
INPUT_DIR  = "./"
...
TRACKER_DIR = os.path.join(INPUT_DIR, '.tracker')
...
def colorjitter(img, cj_type="b"):
    ...
    if cj_type == "b":
        ...
        # value = random.randint(-50, 50)
        value = np.random.choice(np.array([ -40, -30, -20, 20 , 30, 40]))
        # value = -30
        hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
        h, s, v = cv2.split(hsv)
        if value >= 0:
    ...
    for idx, obj in enumerate(img_objects):
        ...
        if pointInRect(mouse_x, mouse_y, x1, y1, x2, y2):
            ...
            if tmp_area < smallest_area or smallest_area == -1:
                ...
                selected_bbox = idx
                if (set_class and len(CLASS_LIST)>1):
...
def convert_video_to_images(video_path, n_frames, desired_img_format):
    ...
    aug_file_path = "augument/"
    if not os.path.exists(aug_file_path): 
        os.makedirs(aug_file_path)
        f = open(aug_file_path+'__augumented__.txt','w')
        f.close()
    video_name_ext = os.path.basename(file_path)
    print(video_name_ext)
    f_read = open(aug_file_path+'__augumented__.txt','r')
    vids = f_read.readlines()
    f_read.close()
    if not os.path.exists(file_path):
        ...
        for i in tqdm(range(n_frames)):
            ...
            ret, frame = cap.read()
            # print("Before:"+str(frame.shape[0])+'x'+str(frame.shape[1])) # shape[0]=height and shape[1]= width
            # print(frame.shape)
            if(frame.shape[0]>frame.shape[1]):
                frame = cv2.rotate(frame,cv2.ROTATE_180)
                if(frame.shape[1]>1080 or frame.shape[0]>1920):
                    if(frame.shape[1]>1080 and frame.shape[0]>1920):
                        frame = cv2.resize(frame,(1080,1920),interpolation = cv2.INTER_AREA)                
                    elif(frame.shape[0]>1920):
                        frame = cv2.resize(frame,(frame.shape[1],1920),interpolation = cv2.INTER_AREA)
                    elif(frame.shape[1]>1080):
                        frame = cv2.resize(frame,(1080,frame.shape[0]),interpolation = cv2.INTER_AREA)
            else:
                if(frame.shape[0]>1080 or frame.shape[1]>1920):
                    if(frame.shape[0]>1080 and frame.shape[1]>1920):    
                        frame = cv2.resize(frame,(1920,1080),interpolation = cv2.INTER_AREA)
                    elif(frame.shape[0]>1080):
                        frame = cv2.resize(frame,(frame.shape[1],1080),interpolation = cv2.INTER_AREA)
                    elif(frame.shape[1]>1920):
                        frame = cv2.resize(frame,(1920,frame.shape[0]),interpolation = cv2.INTER_AREA)
            if ret == True and i%STEPS==0:   #skip frames
                ...
                if i%15==0 and (video_name_ext+'\n') not in vids:
                    bright = os.path.join(aug_file_path,"brightness/")
                    contrast = os.path.join(aug_file_path,"contrast/")
                    saturation = os.path.join(aug_file_path,"saturation/")
                    vertical_flip = os.path.join(aug_file_path,"vertical_flip/")
                    horizontal_flip = os.path.join(aug_file_path,"horizontal_flip/")

                    aug = np.random.choice(np.array([1,2,3,4,5]))
                    if aug == 1 :
                        frame_aug = colorjitter(frame,cj_type="b" )
                        if not os.path.exists(bright): os.makedirs(bright) 
                        final_path= bright # Brightness Augumentation #
                    if aug == 2 :
                        frame_aug = colorjitter(frame,cj_type="c" )
                        if not os.path.exists(contrast): os.makedirs(contrast)
                        final_path= contrast # Contrast Augumentation #   
                    if aug == 3 :
                        frame_aug = colorjitter(frame,cj_type="s" )
                        if not os.path.exists(saturation): os.makedirs(saturation)
                        final_path= saturation # Saturation Augumentation #
                    if aug == 4 :
                        frame_aug = flip(frame,h=0)
                        if not os.path.exists(vertical_flip): os.makedirs(vertical_flip)
                        final_path=vertical_flip # Vertical Flip augmentation Augumentation #
                    if aug == 5 :
                        frame_aug = flip(frame,h=1)
                        if not os.path.exists(horizontal_flip): os.makedirs(horizontal_flip)
                        final_path = horizontal_flip # Hrizontal Flip augmentation Augumentation #
                    frame_aug_name =  '{}_{}{}'.format(video_name_ext, i+1, desired_img_format)
                    frame_aug_path = os.path.join(final_path, frame_aug_name)
                    cv2.imwrite(frame_aug_path,frame_aug)
        # release the video capture object
        print("Image Size:"+str(frame.shape[0])+'x'+str(frame.shape[1]))
        cap.release()
        if (video_name_ext+'\n') not in vids:
            f = open(aug_file_path+'__augumented__.txt','a')
            f.write(video_name_ext+'\n')
            f.close()
    return file_path, video_name_ext, n_frames
...
def get_annotation_paths(img_path, annotation_formats):
    ...
    for ann_dir, ann_ext in annotation_formats.items():
        new_path = os.path.join(INPUT_DIR, ann_dir)
...
def detection(cfg,weights):
    ite=0
    while(ite<f_frames):
            # imgpt.append(IMAGE_PATH_LIST[ite])
            img_p = IMAGE_PATH_LIST[ite]
            format = {'YOLO_darknet' : '.txt'}
            for ann_p in get_annotation_paths(img_p, format): print(ann_p)
            img_d = cv2.imread(img_p)
            print("Image:"+str(ite))
            print(n_frames)
            print(img_p)
            print(CLASS_LIST)
            ...
            classIds, scores, boxes = model.detect(img_d, confThreshold=0.30, nmsThreshold=0.4)
            i=0
            j=0
            k=10
            for (classId, score, box) in zip(classIds, scores, boxes):
                xmin,ymin,xmax,ymax = box[0]-k,box[1]-k,(box[0]+box[2])+k,(box[1]+box[3]+k)
                bw,bh = (xmax-xmin)/img_d.shape[1], (ymax-ymin)/img_d.shape[0]
                diag= math.dist((xmin,ymin),(xmax,ymax))
                xc=((xmax+xmin)/2)/img_d.shape[1]
                yc=((ymax+ymin)/2)/img_d.shape[0]  
                if(diag >= 100):
                    # cv2.rectangle(img, (xmin, ymin), (xmax, ymax),color=(0,0,255), thickness= 2)
                    # cv2.circle(img, (xc.astype(int),yc.astype(int)), radius=10, color=(0, 0, 255), thickness=-1)
                    # text = '%s:%s: %.2f' % (j,classes[classId], score)
                    # print(j)
                    yolo = '%s %s %s %s %s'%(classId,xc,yc,bw,bh)
                    # print(yolo) #Yolo TXT format
                    # cv2.putText(img, text, (box[0],box[1] -5), cv2.FONT_HERSHEY_SIMPLEX, 1, color= (0+i,255-i,0), thickness= 2)
                    i+=25
                    j+=1 
                    append_bb(ann_p,yolo,'.txt')     
            ite=ite+N_FRAMES+1
    f=open(detect_path,'w')
    f.write('1')
    f.close()
    return '1'

def detection_img(cfg,weights):
    ite=0
...
if __name__ == '__main__':
    ...
    for f in sorted(os.listdir(INPUT_DIR), key = natural_sort_key):
        ...
        if test_img is not None:
            ...
            f_frames= n_frames/STEPS
            test_video_cap.release()
            if n_frames > 0:
                # it is a video
                desired_img_format = '.png'
                video_frames_path, video_name_ext, frames = convert_video_to_images(f_path, n_frames, desired_img_format)
    ...
    for ann_dir in annotation_formats:
        new_dir = os.path.join(INPUT_DIR, ann_dir)
        ...
        detect_path = os.path.join(new_dir,'__detect__.txt')
    ...
    # imgpt =list(IMAGE_PATH_LIST[0])
    if(detect=='0' and CFG is not NULL and WEIGHTS is not NULL):
        if(video == 1):
            print('DETECTING')
            detect=detection(CFG, WEIGHTS)
            print('DETECTED')
        else:
            print('DETECTING')
            detect=detection_img(CFG, WEIGHTS)
            print('DETECTED')
    ...
    cv2.resizeWindow(WINDOW_NAME, int(win_width), int(win_height))
    ...
    while True:
        ...
        if(detect=='1'):
            ...
            if(video==1):
                while(itr<f_frames):
                    if(img_index==itr):
                        tmp_img = cv2.putText(tmp_img, "Prdiected",(50, 50), font, 1, color, LINE_THICKNESS, cv2.LINE_AA)
                    itr = itr + N_FRAMES+1
            else:
                 tmp_img = cv2.putText(tmp_img, "Prdiected",(50, 50), font, 1, color, LINE_THICKNESS, cv2.LINE_AA)
        ...
        if dragBBox.anchor_being_dragged is None:
            ''' Key Listeners START '''
            if pressed_key == ord('a') or pressed_key == ord('d') or pressed_key == ord('x') or pressed_key == ord('z'):
            ...
            # help key listener
                ...
                text = ('[e] to show edges;\n'
                        ...
                        '[z] or [x] to jump by defined number of images(Default value:200) \n'
                        )
            ...
            # quit key listener
            elif pressed_key == ord('q') or pressed_key == 27:
                break
            ''' Key Listeners END '''
